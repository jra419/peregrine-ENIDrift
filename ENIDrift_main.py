...
class ENIDrift_train():
    ...
    def __init__(self, hypr=None, delta=None, size=[700, 5, 700, 5],
                 num_test=10, learner='PCA', mute=True, incremental=True, max_subs=30):

        self.G_idx_normal = G_index(hypr[0], delta[0], num_test = size[0], ID='normal')
    ...
    def update(self, y):
        ...
        if attack_temp.shape[0] != 0:
            if self.attack_set.shape[0] == 0:
                ...
                self.attack_set = append(self.attack_set.reshape(-1, 80), attack_temp.reshape(-1, 80), axis=0)
