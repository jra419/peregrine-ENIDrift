...
class G_index():
    ...
    def __init__(
            self, lamda, delta, num_test=1000, kind='PCA', num_simulation=100,
            mute=True, ID='normal'):
        self.ID = ID
        self.delta = delta
        self.lamda = 0.3
    ...
    def check(self, x):
        ...
        if self.G_idx_last < self.G_idx_cur:
            ...
            #   random.permutation(data_test_temp.shape[0])[:self.num_test]]
            if self.data_store.shape[0] != 0:
    ...
    def check_significance(self):
        ...
        for i in range(self.num_test):
            if self.G_idx_cur[i] != 0:
                f_p.append(1 - f.cdf(self.G_idx_last[i] / self.G_idx_cur[i],
                                     self.num_test - 1, self.num_test - 1))
